...
async function getConfiguration(settings) {
    ...
    const etryPromise = oryx.find(settings.entry);
    const modulesPromise = oryx.find(settings.resolveModules, []);

    return await Promise.all([modulesPromise, etryPromise]).then(pathes => {
        ...
        log.error(error, 'imposible to resolve pathes promises');
    });
}
